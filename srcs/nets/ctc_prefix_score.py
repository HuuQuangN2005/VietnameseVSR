...
# Source: https://github.com/nguyenvulebinh/AVSRCocktail/blob/main/src/nets/ctc_prefix_score.py
...
class CTCPrefixScoreTH(object):
    ...
    def __init__(self, x, xlens, blank, eos, margin=0):
        ...
        self.dtype = x.dtype
        self.device = (
            torch.device("cuda:%d" % x.get_device()) if x.is_cuda else torch.device("cpu")
        )
    ...
    def __call__(self, y, state, scoring_ids=None, att_w=None):
        ...
        if self.scoring_num > 0:
            ...
            scoring_idx = (scoring_ids + self.idx_bo.repeat(1, n_hyps).view(-1, 1)).view(
                -1
            )
    ...
    def extend_state(self, state):
        ...
        if state is None:
            ...
            r_prev_new = torch.full(
                (self.input_length, 2), self.logzero, dtype=self.dtype, device=self.device
            )
